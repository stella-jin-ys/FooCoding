...
function fetchPokemonData() {
  ...
  /* Code goes below */
  function showPokemon(arg) {
    const dataObj = JSON.parse(arg);
    const spritesValues = Object.values(dataObj.sprites);
    let rootDiv = document.getElementById("root");
    // let urls=[];
    // urls.push(urls= spritesValues.filter(spritesValue=> spritesValue!==null))
    // urls.forEach(url => {
    ...
    for (let i = 0; i < spritesValues.length; i++) {
      let imgs = document.createElement("img");
      if (spritesValues[i] !== null) {
        imgs.src = `${spritesValues[i]}`;
        rootDiv.appendChild(imgs);
      }
    }
  }
  
  showPokemon(fetchPokemonData());
